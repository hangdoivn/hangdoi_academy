#!/usr/bin/env python3
"""Build Academy blog routes as /blog/<category>/<article>/."""
...
class BlogIndexParser(HTMLParser):
    """Read article slug and category label from generated blog cards."""
...
def load_categories(path: Path) -> tuple[dict[str, str], dict[str, str]]:
    ...
    if not isinstance(categories, list) or not categories:
        raise RouteError("Category config needs a non-empty categories list")
    ...
    for item in categories:
        ...
        for candidate in [label, *aliases]:
            ...
            existing = label_to_slug.get(key)
...
def extract_routes(
    index_html: str, config_path: Path
) -> tuple[list[ArticleRoute], dict[str, str], dict[str, str]]:
    ...
    for article_slug, category_label in parser.articles:
        ...
        key = " ".join(category_label.split()).casefold()
        category_slug = label_to_slug.get(key)
        if not category_slug:
            raise RouteError(
                f"Unknown category {category_label!r} for {article_slug!r}; "
...
def enrich_article(content: str, route: ArticleRoute, base_url: str) -> str:
    ...
    schema = {
        ...
        "itemListElement": [
            ...
            },
            {
                "@type": "ListItem",
                "position": 4,
                "name": article_title(content, route.article_slug),
...
def extract_cards(index_html: str) -> dict[str, str]:
    cards: dict[str, str] = {}
    pattern = re.compile(
        r'<article\b[^>]*class="[^"]*blog-card[^"]*"[^>]*>.*?</article>',
        re.I | re.S,
    )
...
def rewrite_chips(
    ...
    pattern = re.compile(
        r'<(span|a)\b[^>]*class="[^"]*blog-category-chip[^"]*"[^>]*>(.*?)</\1>',
        re.I | re.S,
    )
    ...
    def replacement(match: re.Match[str]) -> str:
        ...
        if not slug or slug not in active_category_slugs:
            return f'<span class="blog-category-chip">{match.group(2)}</span>'
...
def category_page(
    ...
    selected = [cards[item.article_slug] for item in routes if item.article_slug in cards]
    if not selected:
        selected = [
            f'<article class="blog-card"><h2><a href="{item.new_path}">{item.article_slug}</a></h2></article>'
            for item in routes
        ]
    for item in routes:
        selected = [card.replace(item.old_path, item.new_path) for card in selected]
    schema = {
        "@context": "https://schema.org",
        "@graph": [
            {
...
def append_categories_to_sitemap(site_root: Path, slugs: set[str], base_url: str) -> None:
...
def validate(site_root: Path, routes: list[ArticleRoute], active: set[str], base_url: str) -> None:
    index = (site_root / "blog" / "index.html").read_text(encoding="utf-8")
    for slug in active:
        page = site_root / "blog" / slug / "index.html"
        if not page.is_file() or f'href="/blog/{slug}/"' not in index:
            raise RouteError(f"Category route is missing or unlinked: {slug}")
    for route in routes:
        ...
        if not article.is_file() or not redirect.is_file():
            raise RouteError(f"Missing route output for {route.article_slug}")
...
def run(site_root: Path, config_path: Path, base_url: str) -> dict[str, object]:
    ...
    active = set(by_category)

    index_path.write_text(
        rewrite_chips(index_path.read_text(encoding="utf-8"), label_to_slug, active),
        encoding="utf-8",
    )
    ...
    append_categories_to_sitemap(site_root, active, base_url)
    manifest = {
        "version": 1,
        "base_url": base_url,
    ...
    (blog_root / "routes.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    validate(site_root, routes, active, base_url)
    return manifest
